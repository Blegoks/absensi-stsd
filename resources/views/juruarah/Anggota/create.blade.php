...
<div class="bg-white p-6 rounded shadow max-w-lg mx-auto">
    ...
    <input type="text" value="Anggota" class="mt-1 block w-full border rounded px-3 py-2 bg-gray-100" readonly>
</div>
<input type="hidden" name="role" value="anggota">


        {{-- Tempekan (otomatis tampil jika role anggota) --}}
        <div class="mb-4" id="tempekan-field" style="display: none;">
            <label class="block text-sm font-medium text-gray-700">Tempekan</label>
            <input type="text" class="mt-1 block w-full border rounded px-3 py-2 bg-gray-100 text-gray-700" value="{{ $tempekan->nama ?? '-' }}" readonly disabled>
            <input type="hidden" name="tempekan" value="{{ $tempekan->id }}">
            <p class="text-xs text-gray-500 mt-1">Tempekan otomatis diambil dari akun Juru Arah Anda.</p>
        </div>

        {{-- Password Info --}}
        <div class="mb-4">
            <label class="block text-sm font-medium text-gray-700">Password</label>
            <input type="text" class="mt-1 block w-full border rounded px-3 py-2 bg-gray-100 text-gray-400 cursor-not-allowed" value="(Akan dikirim otomatis ke email)" disabled>
